...
$d = Invoke-RestMethod -Uri "$base/diagnose" -Method Post -Body $proj -ContentType "application/json"
...
$fix = Invoke-RestMethod -Uri "$base/diagnose/fix" -Method Post -Body (@{ project = ($proj | ConvertFrom-Json) } | ConvertTo-Json -Depth 8) -ContentType "application/json"
Write-Host ("applied=" + $fix.result.applied.Count + " residualHealth=" + $fix.result.residualReport.summary.healthScore)

Write-Host "== discover =="
$disc = Invoke-RestMethod -Uri "$base/discover" -Method Post -Body (@{ provider = "fly"; appIdentifier = "demo-app" } | ConvertTo-Json) -ContentType "application/json"
...
Write-Host "== plan =="
$plan = Invoke-RestMethod -Uri "$base/plan" -Method Post -Body (@{ spec = $spec } | ConvertTo-Json -Depth 12) -ContentType "application/json"
...
Write-Host "== apply =="
$apply = Invoke-RestMethod -Uri "$base/apply" -Method Post -Body (@{ spec = $spec; plan = $plan.plan; approvedBy = "tester-admin" } | ConvertTo-Json -Depth 12) -ContentType "application/json"
...
$tf = Invoke-RestMethod -Uri "$base/terraform/plan" -Method Post -Body (@{ spec = $spec; currentState = @() } | ConvertTo-Json -Depth 12) -ContentType "application/json"
Write-Host ("tf.summary=" + $tf.plan.summary)

Write-Host "== deploy =="
$dep = Invoke-RestMethod -Uri "$base/deploy" -Method Post -Body (@{ appName = "demo"; targetProvider = "fly"; files = @("package.json", "next.config.js"); packageJson = @{ dependencies = @{ next = "^14" } }; requestedBy = "tester"; enableStripe = $true; enableMonitoring = $true } | ConvertTo-Json -Depth 6) -ContentType "application/json"
...
Write-Host "== audit =="
$a = Invoke-RestMethod -Uri "$base/audit" -Method Get
